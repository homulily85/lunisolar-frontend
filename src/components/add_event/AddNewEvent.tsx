...
const AddNewEvent = () => {
    ...
    const [selectedFrequency, setSelectedFrequency] = useState<Option>(frequency[0]);
    const [selectedReminders, setSelectedReminders] = useState<(Option | null)[]>([]);
    ...
    const updateReminder = useCallback((index: number, value: Option | null) => {
    ...
    const availableReminderOptions = useCallback((currentIndex: number) => {
        const selectedKeys = new Set(selectedReminders
    ...
    return (
        <Dialog open={showDialog} onClose={onClose} className="relative z-50 dark:text-gray-200">
            ...
            <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                <DialogPanel
                    ...
                    <form className="w-full">
                        ...
                        <fieldset className="my-4">
                            ...
                            <div className="grid grid-cols-2 my-2 text-lg gap-1 ">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {!isAllDay && <TimePicker name="start-time" id="start-time"
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {!isAllDay && <TimePicker name="end-time" id="end-time"
                                                              className="justify-self-end"/>}
                                    <div className="col-start-2 col-end-3 justify-self-end">
                                        <DatePicker/>
                                    </div>
                                ...
                                <Listbox value={selectedFrequency} onChange={setSelectedFrequency}>
                                    <ListboxButton
                                        className="relative block w-full py-1.5 pr-8 pl-1 text-left border-b border-b-orange-300 focus:not-data-focus:outline-none data-focus:outline-2 data-focus:-outline-offset-2 data-focus:outline-white/25">
                        ...
                        <div className="relative w-full my-4">
                            <fieldset className="text-lg font-bold">Nhắc nhở</fieldset>
                            <div className="grid grid-cols-[8fr_1fr] gap-2">
                                {selectedReminders.map((rem, idx) => (
                                    <Fragment key={rem?.key ?? idx}>
                                        <div>
                                            <Listbox value={rem ?? EMPTY_REMINDER}
                                                     onChange={(v) => updateReminder(idx, v)}>
                                                <ListboxButton
                                                    className="relative block w-full py-1.5 pr-8 pl-1 text-left border-b border-b-orange-300 focus:not-data-focus:outline-none data-focus:outline-2 data-focus:-outline-offset-2 data-focus:outline-white/25">
                                                    {(rem ?? EMPTY_REMINDER).value}
                                                    <Icon
                                                        className="group pointer-events-none absolute top-2.5 right-2.5"
                                                        path={mdiChevronDown} size={1}/>
                                                </ListboxButton>
                                                <ListboxOptions anchor="bottom"
                                                                className="w-(--button-width) border border-white/5 bg-gray-100 dark:bg-gray-700 [--anchor-gap:--spacing(1)] focus:outline-none">
                                                    {availableReminderOptions(idx).map((r) => (
                                                        <ListboxOption key={r.key} value={r}
                                                                       className="group flex cursor-default items-center gap-2 rounded-lg px-3 py-1.5 select-none data-focus:bg-gray-200 dark:data-focus:bg-gray-600">
                                                            {r.value}
                                                        </ListboxOption>
                                                    ))}
